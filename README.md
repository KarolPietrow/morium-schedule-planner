<img height="300" alt="morium-banner" src="https://github.com/user-attachments/assets/c62a921b-ea2a-4457-9e6e-a2a7476cc890" />

# Morium
#### Morium to nieoficjalna aplikacja mobilna dla systemu UMCS Moria.
Wybierz kierunek, a następnie grupy zajęciowe, aby dopasować plan do siebie!
- Wygodny dostęp mobilny z podziałem na dni tygodnia
- Możliwość łączenia w planie zajęć z różnych kierunków lub lat
- Przechowywanie lokalne, odświeżanie danych z API Moria

Aplikacja jest niezależnym, nieoficjalnym projektem i nie jest w żaden sposób powiązana z systemem Moria, ani z Uniwersytetem Marii Curie-Skłodowskiej w Lublinie (UMCS).

Aplikacja wykorzystuje publiczne, ogólnodostępne API Moria. Aplikacja nie zbiera, nie przetwarza, nie przechowuje ani nie udostępnia żadnych danych osobowych na zewnętrznych serwerach.
